...
export class SwipeStackStore extends AbstractStoreService {
  ...
  // DEV
  uidsAlreadySeen: string[] = [];

  temporaryDuplicateAndAlreadySeenFilter(
    response: generateSwipeStackResponse
  ): uidChoiceMap[] {
    return (response?.users ?? [])
      .filter((m, index, self) => index === self.findIndex((t) => t.uid === m.uid))
      .filter((m) => !this.uidsAlreadySeen.includes(m.uid));
  }

  storeUidsComingThrough(maps: uidChoiceMap[]): uidChoiceMap[] {
    const newUids = maps.map((m) => m.uid);
    this.uidsAlreadySeen = [...new Set([...this.uidsAlreadySeen, ...newUids])];
    ...
    return of(this.format.searchCriteriaClassToDatabase(SC)).pipe(
      ...
      exhaustMap((request) =>
        this.afFunctions
          ...
          .pipe(
            map((r) => this.temporaryDuplicateAndAlreadySeenFilter(r)),
            map((maps) => this.storeUidsComingThrough(maps)),
      ...
      concatMap((users) =>
        this.swipeOutcomeStore
          .addToSwipeAnswers(users)
          .pipe(mapTo((users ?? []).map((u) => u.uid)))
      )
    );
  }
